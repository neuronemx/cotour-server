...
const avStopIcon = '<svg viewBox="0 0 256 256" aria-hidden="true"><path d="M115.8,10.4c-26.2,3.2-48.6,13.3-67,30.1C23.6,63.4,10,94.2,10,128c0,18.3,3.9,35.2,11.7,51.3c26.1,53.6,87.4,79.4,143.8,60.5c34.9-11.7,62.7-39.5,74.3-74.4c8.6-25.6,8.2-53.4-1.1-78.2c-15.4-41.2-52.1-70.6-95.4-76.3C137.3,10,120.6,9.8,115.8,10.4z M143.1,20.4c40.9,6,74.9,34.2,87.8,73.2c13.1,39.2,3,81.8-26.2,111.1c-41.7,41.7-108.6,42.6-151.2,2.1c-18.7-17.8-30.3-40.5-33.4-66c-0.9-7.2-0.8-20.1,0.3-27.7C26.1,72,54.7,37.7,94.1,24.7c6.4-2.1,16.4-4.3,22-4.9C121.9,19.2,137.4,19.5,143.1,20.4z M81.6,81l-1.5,1.4v45.3c0,45,0,45.4,1.2,46.9l1.2,1.5h45.4c44.5,0,45.3,0,46.5-1.2s1.2-1.9,1.2-47.1v-46l-1.5-1.2c-1.5-1.2-1.9-1.2-46.3-1.2H83L81.6,81z M166.1,127.8v38.9h-38.3H89.5v-38.9V88.9h38.3h38.3V127.8L166.1,127.8z"/></svg>';
const avLoopIcon = '<svg viewBox="0 0 256 256" aria-hidden="true"><path d="M224.3,75.7c-14-14-32.5-21.6-52.3-21.6H94.5V39.2c0-4.7-3.3-6.5-7.2-3.9L51.5,58.4c-4,2.6-3.9,6.7,0.1,9.2l35.5,22c4,2.5,7.3,0.7,7.3-4.1V70.8h77.6c31.6,0,57.3,25.7,57.3,57.3c0,5.2-0.7,10.4-2.1,15.4c-1.2,4.4,1.4,9,5.8,10.3c0.7,0.2,1.5,0.3,2.2,0.3c3.7,0,7-2.4,8-6.1c1.8-6.4,2.7-13.1,2.7-19.8C246,108.3,238.3,89.7,224.3,75.7z M210.6,188.4l-35.5-22c-4-2.5-7.3-0.7-7.3,4.1v14.8H83.9c-31.6,0-57.3-25.7-57.3-57.2c0-11.1,3.2-21.8,9.1-31c2.5-3.9,1.4-9-2.5-11.5c-3.9-2.5-9-1.4-11.5,2.5C14.1,99.8,10,113.7,10,128c0,19.7,7.7,38.3,21.6,52.3c14,14,32.5,21.6,52.3,21.6h83.8v14.8c0,4.7,3.3,6.5,7.2,3.9l35.7-23.1C214.6,195.1,214.6,190.9,210.6,188.4z"/></svg>';
const avVolumeIcon = '<svg viewBox="0 0 256 256" aria-hidden="true"><path d="M141.8,104.5c-4.3,0.7-9.5,3.4-12.7,6.7c-3.4,3.4-5.2,6.7-6.4,11.6l-0.9,3.6l-53.3,0.1c-29.3,0-54.4,0.1-55.6,0.1c-2.4,0-3.2,0.7-2.6,2.3c0.2,0.6,7.1,0.7,55.9,0.8l55.6,0.1l0.9,3.6c1.2,4.8,3.1,8.1,6.4,11.6c3.4,3.4,8.5,6,13.5,6.7c12.3,1.9,24.4-6.8,26.8-19.2l0.6-2.9h37.8c33.1,0,37.9-0.1,38.1-0.7c0.5-1.3-0.2-2.2-1.9-2.3c-0.8,0-17.8,0-37.8,0l-36.2,0l-0.8-3.3c-1.1-4.8-3.2-8.6-6.4-11.8c-3.3-3.4-8.6-6.2-13.2-6.9C146,103.9,145.5,103.9,141.8,104.5z"/></svg>';
function renderAudiovisualPanel() {
  ...
  const cards = audiovisualResources.length ? audiovisualResources.map((item) => {
    ...
    const controls = active ? '<span class="audiovisual-inline-controls"><button data-av-stop title="Detener">' + avStopIcon + '</button><button data-av-loop class="' + (audiovisualState.loop ? 'is-active' : '') + '" title="Loop">' + avLoopIcon + '</button><button data-av-volume title="Silenciar o activar volumen">' + avVolumeIcon + '</button></span>' : '';
  ...
  audiovisualPanel.querySelector("[data-av-volume]")?.addEventListener("click", (event) => { event.stopPropagation(); socket.emit("audiovisual:control", { action: "volume", volume: audiovisualState.volume > 0 ? 0 : 1 }); });
...
function interactionListMarkup(hasActive) { if (!interactions.length) return ""; return '<div class="interaction-picker" role="listbox" aria-label="Interacciones disponibles">' + interactions.map((item) => { const selected = String(item.id) === String(selectedInteractionId); const disabled = hasActive ? ' disabled' : ''; return '<button type="button" class="interaction-choice ' + (selected ? 'is-selected' : '') + '" data-interaction-select="' + item.id + '" aria-selected="' + selected + '" role="option"' + disabled + '><span class="interaction-choice-title">' + (item.title || item.prompt || 'Interacción') + '</span><span class="interaction-choice-prompt">' + (item.prompt || item.title || 'Elige una opción') + '</span></button>'; }).join("") + '</div>'; }
