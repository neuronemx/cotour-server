const DEFAULT_DURATION_MS = 60000;
const INPUT_WINDOW_MS = 100;
const CONTROL_ROLES = new Set(["presenter", "stage"]);

function clamp(value, min, max) {
  return Math.max(min, Math.min(max, value));
}

function createGameId(nowMs = Date.now()) {
  return "breakout_" + nowMs.toString(36) + "_" + Math.random().toString(36).slice(2, 8);
}

function createBlocks(columns = 7, rows = 4) {
  const blocks = [];
  const gap = 0.012;
  const marginX = 0.07;
  const top = 0.09;
  const blockWidth = (1 - marginX * 2 - gap * (columns - 1)) / columns;
  const blockHeight = 0.055;
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      blocks.push({
        id: `b_${row}_${column}`,
        x: marginX + column * (blockWidth + gap),
        y: top + row * (blockHeight + gap),
        width: blockWidth,
        height: blockHeight,
        active: true
      });
    }
  }
  return blocks;
}

function createInitialState(nowMs, durationMs = DEFAULT_DURATION_MS) {
  return {
    id: createGameId(nowMs),
    type: "breakout",
    status: "idle",
    duration_ms: durationMs,
    started_at_ms: null,
    ends_at_ms: null,
    remaining_ms: durationMs,
    score: 0,
    misses: 0,
    paddle: { x: 0.5, width: 0.22, speed: 0 },
    ball: { x: 0.5, y: 0.72, vx: 0.22, vy: -0.3, radius: 0.016 },
    blocks: createBlocks(),
    input: { left: 0, right: 0, window_started_at_ms: nowMs },
    last_input_by_audience: new Map(),
    updated_at_ms: nowMs
  };
}

class BreakoutStore {
  constructor(options = {}) {
    this.now = options.now || Date.now;
    this.sessions = new Map();
    this.inputWindowMs = options.inputWindowMs || INPUT_WINDOW_MS;
    this.durationMs = options.durationMs || DEFAULT_DURATION_MS;
  }

  getSession(sessionId) {
    const key = String(sessionId || "");
    if (!this.sessions.has(key)) this.sessions.set(key, createInitialState(this.now(), this.durationMs));
    return this.sessions.get(key);
  }

  hasActive(sessionId) {
    const status = this.getSession(sessionId).status;
    return status === "running" || status === "paused";
  }

  start(sessionId, role, nowMs = this.now()) {
    if (!CONTROL_ROLES.has(String(role || ""))) return { ok: false, reason: "unauthorized_role" };
    const game = createInitialState(nowMs, this.durationMs);
    game.status = "running";
    game.started_at_ms = nowMs;
    game.ends_at_ms = nowMs + game.duration_ms;
    game.remaining_ms = null;
    this.sessions.set(String(sessionId || ""), game);
    return { ok: true, game };
  }

  pause(sessionId, role, nowMs = this.now()) {
    const game = this.getSession(sessionId);
    if (!CONTROL_ROLES.has(String(role || ""))) return { ok: false, reason: "unauthorized_role" };
    if (game.status !== "running") return { ok: false, reason: "invalid_state" };
    game.remaining_ms = Math.max(0, game.ends_at_ms - nowMs);
    game.ends_at_ms = null;
    game.status = "paused";
    game.updated_at_ms = nowMs;
    return { ok: true, game };
  }

  resume(sessionId, role, nowMs = this.now()) {
    const game = this.getSession(sessionId);
    if (!CONTROL_ROLES.has(String(role || ""))) return { ok: false, reason: "unauthorized_role" };
    if (game.status !== "paused") return { ok: false, reason: "invalid_state" };
    game.ends_at_ms = nowMs + Math.max(0, game.remaining_ms || 0);
    game.remaining_ms = null;
    game.status = "running";
    game.updated_at_ms = nowMs;
    return { ok: true, game };
  }

  close(sessionId, role, nowMs = this.now()) {
    const game = this.getSession(sessionId);
    if (!CONTROL_ROLES.has(String(role || ""))) return { ok: false, reason: "unauthorized_role" };
    game.status = "closed";
    game.ends_at_ms = null;
    game.remaining_ms = 0;
    game.updated_at_ms = nowMs;
    return { ok: true, game };
  }

  input(sessionId, audienceId, direction, nowMs = this.now()) {
    const game = this.getSession(sessionId);
    if (game.status !== "running") return { ok: false, reason: "game_not_running" };
    const id = String(audienceId || "");
    if (!id) return { ok: false, reason: "missing_audience_id" };
    if (direction !== "left" && direction !== "right") return { ok: false, reason: "invalid_direction" };
    const lastAt = game.last_input_by_audience.get(id) || 0;
    if (nowMs - lastAt < 60) return { ok: false, reason: "rate_limited" };
    game.last_input_by_audience.set(id, nowMs);
    game.input[direction] += 1;
    return { ok: true };
  }

  applyCollectiveInput(game, nowMs) {
    if (nowMs - game.input.window_started_at_ms < this.inputWindowMs) return;
    const delta = game.input.right - game.input.left;
    game.paddle.speed = clamp(delta * 0.18, -0.9, 0.9);
    game.input.left = 0;
    game.input.right = 0;
    game.input.window_started_at_ms = nowMs;
  }

  step(sessionId, dtMs = 16, nowMs = this.now()) {
    const game = this.getSession(sessionId);
    if (game.status !== "running") return this.snapshot(sessionId, nowMs);
    if (Number.isFinite(game.ends_at_ms) && nowMs >= game.ends_at_ms) {
      game.status = "finished";
      game.remaining_ms = 0;
      game.ends_at_ms = null;
      game.updated_at_ms = nowMs;
      return this.snapshot(sessionId, nowMs);
    }

    this.applyCollectiveInput(game, nowMs);
    const dt = clamp(dtMs, 0, 50) / 1000;
    game.paddle.x = clamp(game.paddle.x + game.paddle.speed * dt, game.paddle.width / 2, 1 - game.paddle.width / 2);

    const ball = game.ball;
    ball.x += ball.vx * dt;
    ball.y += ball.vy * dt;
    if (ball.x - ball.radius <= 0 && ball.vx < 0) ball.vx *= -1;
    if (ball.x + ball.radius >= 1 && ball.vx > 0) ball.vx *= -1;
    if (ball.y - ball.radius <= 0 && ball.vy < 0) ball.vy *= -1;

    const paddleTop = 0.9;
    const paddleLeft = game.paddle.x - game.paddle.width / 2;
    const paddleRight = game.paddle.x + game.paddle.width / 2;
    if (ball.vy > 0 && ball.y + ball.radius >= paddleTop && ball.y < paddleTop + 0.04 && ball.x >= paddleLeft && ball.x <= paddleRight) {
      ball.y = paddleTop - ball.radius;
      ball.vy = -Math.abs(ball.vy);
      ball.vx = clamp(ball.vx + (ball.x - game.paddle.x) * 0.9, -0.55, 0.55);
    }

    for (const block of game.blocks) {
      if (!block.active) continue;
      const hit = ball.x + ball.radius >= block.x && ball.x - ball.radius <= block.x + block.width && ball.y + ball.radius >= block.y && ball.y - ball.radius <= block.y + block.height;
      if (!hit) continue;
      block.active = false;
      game.score += 100;
      ball.vy *= -1;
      break;
    }

    if (ball.y - ball.radius > 1) {
      game.misses += 1;
      ball.x = 0.5;
      ball.y = 0.72;
      ball.vx = game.misses % 2 ? -0.22 : 0.22;
      ball.vy = -0.3;
    }

    if (!game.blocks.some((block) => block.active)) {
      game.status = "finished";
      game.remaining_ms = Math.max(0, game.ends_at_ms - nowMs);
      game.ends_at_ms = null;
    }
    game.updated_at_ms = nowMs;
    return this.snapshot(sessionId, nowMs);
  }

  snapshot(sessionId, nowMs = this.now()) {
    const game = this.getSession(sessionId);
    const remaining = game.status === "running" && Number.isFinite(game.ends_at_ms)
      ? Math.max(0, game.ends_at_ms - nowMs)
      : Math.max(0, game.remaining_ms || 0);
    return {
      id: game.id,
      type: game.type,
      status: game.status,
      duration_ms: game.duration_ms,
      remaining_ms: remaining,
      score: game.score,
      misses: game.misses,
      paddle: { ...game.paddle },
      ball: { ...game.ball },
      blocks: game.blocks.map((block) => ({ ...block })),
      updated_at_ms: game.updated_at_ms
    };
  }
}

module.exports = {
  BreakoutStore,
  DEFAULT_DURATION_MS,
  INPUT_WINDOW_MS,
  CONTROL_ROLES,
  createBlocks,
  createInitialState
};