...
test("external modal close paths are blocked only when active modals are already open", () => { const source = readProjectFile("public/shared/raffle-controller.js"); assert.match(source, /function isRaffleNavigationLocked\(state\) \{ return Boolean\(state\?\.active\); \}/); assert.match(source, /function isInteractionNavigationLocked\(state\) \{ return Boolean\(state\?\.active \|\| state\?\.activeInteraction\); \}/); assert.match(source, /function installNavigationLockGuards\(\)/); assert.match(source, /event\.key === "Escape" && isInteractionNavigationLocked\(state\)/); assert.match(source, /isInteractionNavigationLocked\(state\) && isExternalCloseTarget\(event\.target\)/); assert.match(source, /target\?\.closest\?\.\("#interactionToggle"\)\) return isPresenterInteractionPanelOpen\(\)/); assert.match(source, /target\?\.closest\?\.\("#stageActionsButton"\)\) return isStageActionsModalOpen\(\)/); assert.match(source, /\[data-interaction-panel-close\], \[data-stage-actions-close\], \[data-close-modal\]/); assert.match(source, /addEventListener\("keydown",[\s\S]+true\)/); assert.match(source, /addEventListener\("click",[\s\S]+true\)/); });
