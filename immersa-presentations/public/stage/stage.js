...
document.getElementById("messageForm").addEventListener("submit", (event) => { event.preventDefault(); const text = messageInput.value.trim(); if (text) updateOverlay({ messageVisible: true, messageText: text, standbyMode: false }); });
document.getElementById("clearMessage").addEventListener("click", () => { messageInput.value = ""; socket.emit("clear_message"); });
document.getElementById("clearScreen").addEventListener("click", () => socket.emit("clear_screen"));
