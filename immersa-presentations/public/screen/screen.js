...
const socket = io();
let manifest = null;
let overlays = { reactionsOnScreen: false, qrVisible: false, messageVisible: false, messageText: "", standbyMode: false };
...
function applyOverlays(next) { overlays = { ...overlays, ...next }; qr.classList.toggle("hidden", !overlays.qrVisible || overlays.standbyMode); message.textContent = overlays.messageText || ""; message.classList.toggle("hidden", !overlays.messageVisible || !overlays.messageText); screen.classList.toggle("standby", !!overlays.standbyMode); }
...
socket.on("clear_overlays", () => applyOverlays({ reactionsOnScreen: false, qrVisible: false, messageVisible: false, messageText: "", standbyMode: false }));
