...
function applyOverlays(next) { overlays = { ...overlays, ...next }; if (overlays.audienceUrl && overlays.audienceUrl !== activeAudienceUrl) { activeAudienceUrl = overlays.audienceUrl; makeQrPattern(activeAudienceUrl); } qr.classList.toggle("hidden", !overlays.qrVisible); message.textContent = overlays.messageText || ""; message.classList.toggle("hidden", !overlays.messageVisible || !overlays.messageText); }
