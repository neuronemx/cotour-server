(function (root, factory) {
  ...
  function create(socket, options = {}) {
    ...
    function scheduleResync() { clearResync(); if (!joined || !socket.connected || state?.timer?.status !== "running") return; resyncTimer = setTimer(() => { resyncTimer = null; synchronize(1).catch(() => {}); }, options.resyncMs || RESYNC_MS); resyncTimer?.unref?.(); }
