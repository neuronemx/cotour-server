(function (root, factory) {
  ...
  function normalizeControllerState(payload) { return { active: payload?.active || null, previousWinnerCount: Number(payload?.previousWinnerCount || 0) }; }
  ...
  function renderVisualKeyPreview() { return '<div class="raffle-visual-preview" aria-label="Vista previa de claves visuales temporales">' + TEMP_VISUAL_KEY_OPTIONS.map((option) => '<span class="raffle-visual-card is-' + option.tone + '" aria-hidden="true">' + escapeHtml(option.label.replace("Clave ", "")) + '</span>').join("") + '</div>'; }
  function renderSelection(state) { const disabled = Boolean(state.activeInteraction || state.pendingEvent); const warning = state.activeInteraction ? '<p class="raffle-warning">Cierra la encuesta activa antes de iniciar un sorteo.</p>' : ""; const buttons = RAFFLE_MODES.map((mode) => '<button type="button" class="raffle-mode-card" data-raffle-create="' + mode.id + '" ' + (disabled ? 'disabled' : '') + '><strong>' + mode.label + '</strong>' + (mode.description ? '<span class="raffle-mode-description">' + escapeHtml(mode.description) + '</span>' : "") + (mode.id === "visual_key" ? renderVisualKeyPreview() : "") + '</button>').join(""); return '<section class="raffle-section"><div class="raffle-heading"><span>Sorteos</span><h2>Sorteo</h2><p>Elige un modo para crear la convocatoria.</p></div>' + warning + '<div class="raffle-mode-grid">' + buttons + '</div></section>'; }
