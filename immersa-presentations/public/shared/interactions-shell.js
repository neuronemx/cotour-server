(function (root, factory) {
  ...
  const CATEGORIES = [
    ...
    { id: "contests", label: "Concursos", enabled: false },
    { id: "games", label: "Juegos", enabled: false }
  ];
  ...
  function create(options = {}) {
    ...
    CATEGORIES.forEach((category) => { const button = createElement(documentRef, "button", "interactions-shell-category", category.label); button.type = "button"; button.dataset.interactionsCategory = category.id; button.disabled = !category.enabled; if (!category.enabled) button.setAttribute("aria-disabled", "true"); nav.appendChild(button); buttons.set(category.id, button); });
    const home = createElement(documentRef, "div", "interactions-shell-home");
    home.innerHTML = '<p>Elige una categoría para iniciar una interacción.</p>';
    ...
    function renderState() {
      ...
      buttons.forEach((button, id) => { const active = id === view; button.classList.toggle("is-active", active); button.setAttribute("aria-pressed", String(active)); if (button.getAttribute("aria-disabled") === "true") button.disabled = true; else button.disabled = locked && !active; });
