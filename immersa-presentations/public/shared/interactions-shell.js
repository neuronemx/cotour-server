(function (root, factory) {
  ...
  const VIEWS = new Set(["home", "polls", "raffles", "qna", "games"]);
  ...
  const CATEGORIES = [
    ...
    { id: "qna", label: "Preguntas", enabled: true, icon: APPROVED_CATEGORY_ICONS.qna },
  ...
  function create(options = {}) {
    ...
    CATEGORIES.forEach((category) => {
      const button = createElement(documentRef, "button", "interactions-shell-category");
      button.type = "button";
      button.dataset.interactionsCategory = category.id;
      const icon = documentRef.createElementNS ? documentRef.createElementNS("http://www.w3.org/2000/svg", "svg") : createElement(documentRef, "svg");
      icon.setAttribute("viewBox", category.icon.viewBox);
      ...
      category.icon.rects.forEach((rectAttrs) => {
        const rect = documentRef.createElementNS ? documentRef.createElementNS("http://www.w3.org/2000/svg", "rect") : createElement(documentRef, "rect");
        Object.keys(rectAttrs).forEach((name) => rect.setAttribute(name, rectAttrs[name]));
        icon.appendChild(rect);
      });
      category.icon.paths.forEach((pathData) => {
        const path = documentRef.createElementNS ? documentRef.createElementNS("http://www.w3.org/2000/svg", "path") : createElement(documentRef, "path");
        path.setAttribute("d", pathData);
        icon.appendChild(path);
      });
      const label = createElement(documentRef, "span", "interactions-shell-category-label", category.label);
      button.append(icon, label);
      button.disabled = !category.enabled;
      button.hidden = options.categoryVisibility?.[category.id] === false;
      if (!category.enabled) { button.setAttribute("aria-disabled", "true"); button.tabIndex = -1; }
    ...
    function syncRendererVisibility() {
      content.querySelectorAll?.("[data-interactions-view]").forEach((node) => { node.hidden = node.dataset.interactionsView !== view; });
    }
    ...
    listen(nav, "click", (event) => { const button = event.target?.closest?.("[data-interactions-category]"); if (!button || !nav.contains(button) || button.disabled || button.hidden || locked) return; const nextView = button.dataset.interactionsCategory; if (!VIEWS.has(nextView)) return; setView(nextView); options.onSelectCategory?.(nextView); });
    ...
    renderState();
    return { setView, setCategoryVisible, refreshRendererVisibility: syncRendererVisibility, setLocked(value) { locked = Boolean(value); renderState(); }, setCloseVisible(value) { closeVisible = Boolean(value); renderState(); }, setTitleVisible(value) { titleVisible = Boolean(value); renderState(); }, getContentRoot() { return content; }, getView() { return view; }, destroy() { if (destroyed) return; destroyed = true; listeners.splice(0).forEach(([target, eventName, handler]) => target.removeEventListener(eventName, handler)); container.remove(); } };
  }
  return { create };
});
