...
function attachInteractionCloseSlider(control, interactionId) {
  ...
  let maxX = 1;
  let progress = 0;
  const reset = () => { progress = 0; control.style.setProperty("--close-progress", "0"); control.classList.remove("is-complete"); };
  const update = (clientX) => { progress = Math.max(0, Math.min(1, (clientX - startX) / maxX)); control.style.setProperty("--close-progress", String(progress)); control.classList.toggle("is-complete", progress >= 0.82); };
...
function renderInteractionPanel() { const panel = ensureInteractionPanel(); const active = activeInteraction || null; const selected = selectedInteraction(); const launchTarget = active || selected; const hasActive = Boolean(active); const revealLabel = interactionResultsVisible ? 'Ocultar resultados' : 'Mostrar resultados'; const heading = hasActive ? '<div class="interaction-panel-heading"><span>' + (interactionResultsVisible ? 'Resultados en pantalla' : 'Encuesta activa') + '</span><h2>' + (launchTarget?.title || 'Encuesta') + '</h2></div>' : '<div class="interaction-panel-heading"><h2>Interacciones</h2></div>'; const body = launchTarget ? '<p>' + (hasActive ? (launchTarget.prompt || launchTarget.title || 'Interacción') : 'Encuestas disponibles') + '</p><p>' + (hasActive ? responseCountText(interactionResults) : 'Selecciona una encuesta para lanzarla.') + '</p>' + interactionListMarkup(hasActive) : '<p>Este deck aún no tiene interacciones.</p>'; const closeControl = hasActive ? '<div class="interaction-close-slider" data-interaction-close-slider role="button" aria-label="Desliza para cerrar encuesta" tabindex="0" style="--close-progress:0"><span class="interaction-close-slider-track"></span><span class="interaction-close-slider-label">Desliza para cerrar encuesta</span><span class="interaction-close-slider-knob" aria-hidden="true">›</span></div>' : '<div class="interaction-close-slider is-disabled" aria-disabled="true"><span class="interaction-close-slider-track"></span><span class="interaction-close-slider-label">Desliza para cerrar encuesta</span><span class="interaction-close-slider-knob" aria-hidden="true">›</span></div>'; panel.innerHTML = heading + body + '<div class="interaction-panel-actions"><button class="primary" data-interaction-launch ' + (!selected || hasActive ? 'disabled' : '') + '>Lanzar encuesta</button><button data-interaction-reveal ' + (!hasActive ? 'disabled' : '') + '>' + revealLabel + '</button></div>' + closeControl + resultRows(interactionResults); panel.querySelectorAll("[data-interaction-select]").forEach((button) => button.addEventListener("click", () => { if (hasActive) return; selectedInteractionId = button.dataset.interactionSelect || ""; renderInteractionPanel(); })); panel.querySelector("[data-interaction-launch]")?.addEventListener("click", () => socket.emit("interaction:launch", { interactionId: selected?.id })); panel.querySelector("[data-interaction-reveal]")?.addEventListener("click", () => { const eventName = interactionResultsVisible ? "interaction:hide_results" : "interaction:reveal_results"; socket.emit(eventName, { interactionId: activeInteraction?.id }); }); attachInteractionCloseSlider(panel.querySelector("[data-interaction-close-slider]"), activeInteraction?.id); }
