...
function renderAudiovisualPanel() {
  ...
  const cards = audiovisualResources.length ? audiovisualResources.map((item) => {
    ...
    const controls = active ? '<span class="audiovisual-inline-controls"><button data-av-stop title="Detener">' + avStopIcon + '</button><button data-av-loop class="' + (audiovisualState.loop ? 'is-active' : '') + '" title="Loop">' + avLoopIcon + '</button><label class="av-volume-control"><input data-av-volume type="range" min="0" max="1" step="0.05" value="' + audiovisualState.volume + '"></label></span>' : '';
