...
function renderInteractionPanel() { const panel = ensureInteractionPanel(); const active = activeInteraction || null; const selected = selectedInteraction(); const launchTarget = active || selected; const hasActive = Boolean(active); const revealLabel = interactionResultsVisible ? 'Ocultar resultados' : 'Mostrar resultados'; const stateLabel = hasActive ? (interactionResultsVisible ? 'Resultados en pantalla' : 'Encuesta activa') : 'Interacciones'; const helper = hasActive ? responseCountText(interactionResults) : 'Selecciona una encuesta para lanzarla.'; panel.innerHTML = '<div class="interaction-panel-heading"><span>' + stateLabel + '</span><h2>' + (hasActive ? (launchTarget?.title || 'Encuesta') : 'Interacciones') + '</h2></div>' + (launchTarget ? '<p>' + (hasActive ? (launchTarget.prompt || launchTarget.title || 'Interacción') : 'Encuestas disponibles') + '</p><p>' + helper + '</p>' + interactionListMarkup(hasActive) : '<p>Este deck aún no tiene interacciones.</p>') + '<div class="interaction-panel-actions"><button class="primary" data-interaction-launch ' + (!selected || hasActive ? 'disabled' : '') + '>Lanzar encuesta</button><button data-interaction-reveal ' + (!hasActive ? 'disabled' : '') + '>' + revealLabel + '</button><button class="danger" data-interaction-close ' + (!hasActive ? 'disabled' : '') + '>Cerrar encuesta</button></div>' + resultRows(interactionResults); panel.querySelectorAll("[data-interaction-select]").forEach((button) => button.addEventListener("click", () => { if (hasActive) return; selectedInteractionId = button.dataset.interactionSelect || ""; renderInteractionPanel(); })); panel.querySelector("[data-interaction-launch]")?.addEventListener("click", () => socket.emit("interaction:launch", { interactionId: selected?.id })); panel.querySelector("[data-interaction-reveal]")?.addEventListener("click", () => { const eventName = interactionResultsVisible ? "interaction:hide_results" : "interaction:reveal_results"; socket.emit(eventName, { interactionId: activeInteraction?.id }); }); panel.querySelector("[data-interaction-close]")?.addEventListener("click", () => socket.emit("interaction:close", { interactionId: activeInteraction?.id })); }
