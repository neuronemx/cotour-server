...
function renderAudiovisualPanel() { ensureAudiovisualUi(); const resource = audiovisualState.resource; const progress = audiovisualState.duration > 0 ? Math.max(0, Math.min(100, (Number(audiovisualState.position || 0) / audiovisualState.duration) * 100)) : 0; audiovisualPanel.innerHTML = '<div class="audiovisual-panel-head"><h2>Librería Immersa</h2><span>' + audiovisualResources.length + ' recursos disponibles</span></div><div class="audiovisual-resource-list">' + (audiovisualResources.length ? audiovisualResources.map((item) => { const active = resource?.id === item.id && audiovisualState.status !== "stopped"; return '<div class="audiovisual-resource ' + (active ? 'is-active' : '') + '" data-av-resource="' + item.id + '" role="button" tabindex="0">' + (item.thumbnail_url ? '<img src="' + item.thumbnail_url + '" alt="">' : '<span class="av-icon">♫</span>') + '<span><strong>' + avEscape(item.name) + '</strong></span><span class="av-play">▶</span>' + (active ? '<span class="audiovisual-inline-controls"><button class="av-stop" data-av-stop title="Detener">■</button><button data-av-loop class="' + (audiovisualState.loop ? 'is-active' : '') + '" title="Loop">↻</button><input class="audiovisual-progress" data-av-seek type="range" min="0" max="100" value="' + progress + '" aria-label="Progreso"><input class="audiovisual-volume" data-av-volume type="range" min="0" max="100" value="' + Math.round((audiovisualState.volume ?? 1) * 100) + '" aria-label="Volumen"></span>' : '') + '</div>'; }).join('') : '<p>No hay recursos seleccionados en Librería.</p>') + '</div>';
  ...
  audiovisualPanel.querySelector("[data-av-seek]")?.addEventListener("change", (event) => { event.stopPropagation(); const duration = Number(audiovisualState.duration || 0); if (duration) socket.emit("audiovisual:control", { action: "seek", position: (Number(event.target.value) / 100) * duration }); });
  audiovisualPanel.querySelector("[data-av-volume]")?.addEventListener("input", (event) => { event.stopPropagation(); socket.emit("audiovisual:control", { action: "volume", volume: Number(event.target.value) / 100 }); });
}
function applyAudiovisualState(next = {}) { audiovisualState = { ...audiovisualState, ...next }; renderAudiovisualPanel(); }
setInterval(() => {
  const slider = audiovisualPanel?.querySelector("[data-av-seek]");
  if (!slider || !audiovisualState.duration) return;
  const position = audiovisualState.status === "playing" && audiovisualState.startedAt
    ? Number(audiovisualState.position || 0) + ((Date.now() - audiovisualState.startedAt) / 1000)
    : Number(audiovisualState.position || 0);
  slider.value = String(Math.max(0, Math.min(100, (position / audiovisualState.duration) * 100)));
}, 250);
