...
const avStopIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="7" y="7" width="10" height="10" rx=".8"></rect></svg>';
const avLoopIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 8a7 7 0 0 0-11.2-1.8L5 8"></path><path d="M5 4v4h4"></path><path d="M6 16a7 7 0 0 0 11.2 1.8L19 16"></path><path d="M19 20v-4h-4"></path></svg>';
const avVolumeIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 10v4h3l4 3V7l-4 3Z"></path><path d="M15 10a3 3 0 0 1 0 4"></path><path d="M17.5 7.5a6.5 6.5 0 0 1 0 9"></path></svg>';
function renderAudiovisualPanel() { ensureAudiovisualUi(); const resource = audiovisualState.resource; audiovisualPanel.innerHTML = '<div class="audiovisual-panel-head"><h2>Librería Immersa</h2><span>' + audiovisualResources.length + ' recursos disponibles</span></div><div class="audiovisual-resource-list">' + (audiovisualResources.length ? audiovisualResources.map((item) => { const active = resource?.id === item.id && audiovisualState.status !== "stopped"; return '<div class="audiovisual-resource ' + (active ? 'is-active' : '') + '" data-av-resource="' + item.id + '" role="button" tabindex="0">' + (item.thumbnail_url ? '<img src="' + item.thumbnail_url + '" alt="">' : '<span class="av-icon">♫</span>') + '<span><strong>' + avEscape(item.name) + '</strong></span>' + (active ? '<span class="audiovisual-inline-controls"><button data-av-stop title="Detener">' + avStopIcon + '</button><button data-av-loop class="' + (audiovisualState.loop ? 'is-active' : '') + '" title="Loop">' + avLoopIcon + '</button><label><span class="visually-hidden">Volumen</span>' + avVolumeIcon + '<input data-av-volume type="range" min="0" max="100" value="' + Math.round((audiovisualState.volume ?? 1) * 100) + '" aria-label="Volumen"></label></span>' : '') + '</div>'; }).join('') : '<p>No hay recursos seleccionados en Librería.</p>') + '</div>';
  ...
  const volumeControl = audiovisualPanel.querySelector("[data-av-volume]");
  volumeControl?.closest("label")?.addEventListener("click", (event) => { event.preventDefault(); event.stopPropagation(); socket.emit("audiovisual:control", { action: "volume", volume: audiovisualState.volume > 0 ? 0 : 1 }); });
}
